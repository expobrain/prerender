...
server.createPhantom = function() {
    ...
    var opts = {
        ...
        onExit: function() {
            util.log({
                message: 'phantom crashed, restarting...'
            });
...
server.onPhantomCreate = function(phantom) {
    ...
    this.phantom.id = Math.random().toString(36).slice(2);

    util.log({
        message: 'started phantom',
        pid: this.phantom.id
    });
};
...
server.onRequest = function(req, res) {
    ...
    util.log({
        message: ['getting', req.prerender.url],
        pid: this.phantom ? this.phantom.id : undefined
    });
...
server.checkIfJavascriptTimedOut = function(req, res) {
    ...
    if (!this.phantom || this.phantom.id !== req.prerender.phantomId) {
        util.log({
            level: 'warning',
            message: 'PhantomJS restarted in the middle of this request. Aborting...',
            pid: req.prerender.phantomId
        });
        ...
        util.log({
            level: 'warning',
            message: 'Timed out. Sending request with HTML on the page',
            pid: this.phantom.id
        });
        ...
        util.log({
            level: 'error',
            message: [
                'Experiencing infinite javascript loop on', req.prerender.url,
                'Killing phantomjs...'
            ],
            pid: this.phantom.id
        });
...
server._killPhantomJS = function() {
    this.options.worker.kill();
       ...
       //     util.log({
       //         level: 'error',
       //         message: ['Error killing phantomjs from javascript infinite loop:', e],
       //         pid: this.phantom.id
       //     });
       // }
}
