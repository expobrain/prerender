...
util.log = function (data) {
    ...
    // Skip if logging is disable
    if (process.env.DISABLE_LOGGING) {
       return;
    }

    // Select log method by level
    level = data.level || 'info',
    method = console[level] || console.log;

    // Build message
    msg = [
        new Date().toISOString(),           // Timestamp
        '[ ' + level + ' ]',                // Log level
        '[ worker: ' + (cluster.isMaster ? 0 : cluster.worker.id) + ' ]',  // Worker ID
        '[ ' + (data.pid || 'main') + ' ]', // PhantomJS PID if any
    ];

    if (_.isArray(data.message)) {
        msg = msg.concat(data.message);
    } else {
        msg.push(data.message);
    }

    // Log it
    method.apply(method, msg);
};
